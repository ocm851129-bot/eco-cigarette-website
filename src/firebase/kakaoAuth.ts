...
/* Vercel 서버리스 함수로 code → access_token 교환 */
async function getAccessToken(code: string): Promise<string> {
  const params = new URLSearchParams({ code, redirect_uri: REDIRECT_URI })
  const res    = await fetch(`/api/kakao-token?${params}`)
  const data   = await res.json()

  if (!res.ok || data.error) throw new Error(data.error || '토큰 교환 실패')
...
export async function signInWithKakao(): Promise<void> {
  ...
  // 2. 서버리스 함수로 access_token 교환 (CORS 우회)
